...
RUN apt-get update \
    ...
    && pecl install ev \
                && echo "extension=ev.so" > /etc/php/${PHP_VERSION}/cli/conf.d/ev.ini \
                && echo "extension=ev.so" > /etc/php/${PHP_VERSION}/embed/conf.d/ev.ini \
                && echo "extension=ev.so" > /etc/php/${PHP_VERSION}/mods-available/ev.ini \
    && cd /opt \
